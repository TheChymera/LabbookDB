...
def sucrosepreference_plot(db_path, animals={}, include_relationship_columns=["drinking_measurements","measurements"], normalize=True):
	# include_relationship_columns.append("sucrosepreference_measurements")

	session, engine = loadSession(db_path)

	cols = []
	joins = []
	insp = inspection.inspect(Cage,DrinkingMeasurement)
	...
	# DM=with_polymorphic(Measurement,DrinkingMeasurement, aliased=True)
	# sql_query = session.query(*cols).select_from(Cage, DrinkingMeasurement)
	sql_query = session.query(*cols).select_from(Cage)
	# sql_query = sql_query.outerjoin(DM)
	for join in joins:
		sql_query = sql_query.outerjoin(*join)
		# sql_query = sql_query.options(joinedload_all(*join))

	# sql_query = sql_query.filter(or_(Cage.birth_date == datetime.datetime(*[int(a) for a in birthdate.split(",")]) for birthdate in animals["birth_date"]))
...
if __name__ == '__main__':
	sucrosepreference_plot("~/syncdata/meta.db")
